...
pO = ggplot(orgdata, aes(X1, value, group = as.factor(X2)))+ 
  ...
  ggtitle("Original Data")+ xlab("")+ylim(55,100)+theme_minimal() #changed from alpha=0.05 to .2 to make lines darker
...
pL = ggplot(NvMlr, aes(X1, value, group = as.factor(X2)))+ 
  geom_line(alpha=.1)+ scale_x_continuous()+ 
  ggtitle("Low rank")+ xlab("")+ylim(55,100)+theme_minimal() #changed alpha from 0.05 to 0.2
...
pS = ggplot(NvMSp, aes(X1, value, na.rm=TRUE, group = as.factor(X2)))+ 
...
pS = ggplot(NvMSp, aes(X1, value, na.rm=TRUE, group = as.factor(X2)))+ 
  geom_line(alpha=.2)+ scale_x_continuous()+ 
...
#deleted the absolute value part because it didn't make sense
#SpDBabs=abs(SpDB) #negative values so taking the absolute value
#colnames(SpDBabs) = hix 
#NvMSp <- reshape :: melt(t(SpDB)  )
#pS = ggplot(NvMSp, aes(X1, value, na.rm=TRUE, group = as.factor(X2)))+ 
  #geom_point(alpha=.1)+ scale_x_continuous()+ 
  #ggtitle("Sparse")+ xlab("")+ylim(0,100)+theme_minimal() #changed alpha from .05 to .5 to be able to see these outliers more
...
#improving LR spectrogram
#blue yellow red old
#Lrmatrix=as.matrix(LrDB) #get the LR data (in decibels) into matrix form
#create color map
#colMap <- colorRampPalette(c("blue","yellow","red" ))(Lrmatrix)
#plot
#Lrspectro=image(Lrmatrix, col=colMap) #this should create a spectrogram-- not really sure what axes are here...but it looks like there are 7 choruses!!
#legend(grconvertX(0.5, "device"), grconvertY(1, "device"), 
      # c("0",".5","1"), fill = colMap[c(1,39, 77)], xpd = NA)
#legend(grconvertX(0.5, "device"), grconvertY(1, "device"), 
       #c("1","0.5","0"), fill = colMap[c(77,39, 1)], xpd = NA)
#Lrspectro+title(main="LR")
#Lrspectro + theme(legend.position = c(0.8, 0.2))
...
Spmatrix[Spmatrix == -Inf] <- NaN #get rid of all the -inf values bc sparse matrix is screwed up
SpDBabs=abs(Spmatrix) #negative values so taking the absolute value
Spspectro=image(Spmatrix, col=brewer.pal(n=9,name="YlOrRd")) #looks weird...
...
#now just night-- didn't use this part
...
hix  = as.numeric( gsub("PSD_","",names(nv)[1:48]) ) #ELLA changed from 25 to 17 b/c it was introducing NAs before. and didn't run lines 160-164... do i need those or is that only for truncating step????
...
#######################################################
#save matrices for SDT BF_01 to folder-- this actually just saves the 1 week of data as matrix. go down to next area of code to save matrices for the entire deployment
...
write.csv(Lrnightmatrix, "E:\\SDT\\SDT RRPCA\\RRPCA SDT\\matrices\\Lrnightmatrix_SDT_BF_01.csv",row.names=FALSE)
write.csv(Spnightmatrix, "E:\\SDT\\SDT RRPCA\\RRPCA SDT\\matrices\\Spnightmatrix_SDT_BF_01.csv",row.names=FALSE)
write.csv(Onightmatrix, "E:\\SDT\\SDT RRPCA\\RRPCA SDT\\matrices\\Onightmatrix_SDT_BF_01.csv",row.names=FALSE)
